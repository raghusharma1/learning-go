...
func TestIsValidEmail(t *testing.T) {
	tests := []struct {
		...
		{
			email:    strings.Repeat("a", 256) + "@email.com", // Email length exceeds 255 characters
			expected: false,
		},
		{
			email:    "invalid email", // Email does not match the regular expression pattern
			expected: false,
		},
		{
			email:    "", // Empty email
	...
	for _, test := range tests {
		t.Run(test.email, func(t *testing.T) {
			...
			if result {
				t.Logf("Success: The email '%v' was correctly identified as valid", test.email)
			} else {
				t.Logf("Success: The email '%v' was correctly identified as invalid", test.email)
			}
		})
	}
}
