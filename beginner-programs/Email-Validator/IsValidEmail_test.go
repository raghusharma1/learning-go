...
func TestIsValidEmail(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			email:    "this.email.address.is.clearly.too.long.to.be.valid@example.com",
			expected: false,
		},
	}
	...
	for i, testCase := range testCases {
		t.Logf("Executing test case %d", i+1)
		result := IsValidEmail(testCase.email)
		if result != testCase.expected {
			t.Errorf("Test case %d failed. Expected %v but got %v", i+1, testCase.expected, result)
		} else {
