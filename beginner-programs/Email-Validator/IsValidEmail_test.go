...
func TestIsValidEmail(t *testing.T) {
	testCases := []struct {
		...
		{"userexample", false},                // Scenario 2: Test with an invalid email
		{"", false},                           // Scenario 3: Test with an empty string
		{string('\x00'), false},               // Scenario 4: Test with a null input
		{string(make([]byte, 256)), false},    // Scenario 5: Test with a string larger than 255 characters
	}

	for _, testCase := range testCases {
		result := IsValidEmail(testCase.email)
		if result != testCase.expectedResult {
			t.Errorf("Expected result for email '%s' is '%v', but got '%v'", testCase.email, testCase.expectedResult, result)
		} else {
			t.Logf("Success: Expected result for email '%s' is '%v'", testCase.email, testCase.expectedResult)
		}
	}
}
