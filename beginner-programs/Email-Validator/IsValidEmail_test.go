...
func TestIsValidEmail(t *testing.T) {
	...
	tests := []struct {
		...
		{
			email: "test.example.com", // invalid email address
			want:  false,
		},
		{
			email: "test@example.com" + string(make([]byte, 256)), // excessively long email address
			want:  false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.email, func(t *testing.T) {
			...
			if got != tt.want {
				...
				t.Logf("IsValidEmail(%v) = %v - Test Passed", tt.email, got)
			}
		})
	}
}
